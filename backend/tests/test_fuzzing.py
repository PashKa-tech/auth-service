...
@pytest.mark.asyncio
@given(token=st.text())
async def test_fuzz_verify_access_token(token):
...
@pytest.mark.asyncio
@given(token=st.text())
async def test_fuzz_verify_mfa_token(token):
    """Fuzz verify_mfa_token with random strings to ensure it doesn't crash."""
    result = await verify_mfa_token(token)
    assert result is None
...
async def test_fuzz_hash_and_verify_password(password):
    ...
    try:
        ...
        assert await verify_password(password + "x", hashed) is False
...
async def test_fuzz_verify_password_random_hash(password, hashed):
